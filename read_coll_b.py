...
dir1 = os.getcwd()+'/H_Rb_10GeV_density_Awake/'
...
for i,j in enumerate(x):
    ...
    #print 'Index error at line ' + str(i)
    #pass

elec_energy=[]
for ii in listnew:
    elec_energy.append(np.float(ii.split()[3]))

#elec_energy = np.array(elec_energy)

with open('export_elec_energies.txt','w') as f:
    for jj in elec_energy:
        aa = str(jj).replace('.',',')
        f.write(aa + '\n')
